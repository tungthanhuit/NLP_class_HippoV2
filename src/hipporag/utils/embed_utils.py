...
def retrieve_knn(
    ...
    for query_batch, query_batch_start_idx in tqdm(
        ...
        total=(len(query_vecs) + query_batch_size - 1)
        // query_batch_size,  # Calculate total batches
        ...
        for key_batch, key_batch_start_idx in get_batches(
            vecs=key_vecs, batch_size=key_batch_size
        ):
        ...
        for i in range(final_topk_indices.size(0)):
            ...
            query_to_topk_key_ids = [
                key_ids[idx] for idx in query_to_topk_key_relative_ids.cpu().numpy()
            ]
            results[query_idx] = (
                query_to_topk_key_ids,
                final_topk_sim_scores_i.numpy().tolist(),
            )
